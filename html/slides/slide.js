...
function dslide(id, time) {
	...
	this.htm += '<div style="width:'+(this.w-8)+'px;height:20px;overflow:hidden;z-index:3;position:relative;margin-top:-25px;padding:0 4px 0 4px;text-align:right;">';
	for(var i = 0; i < this.max; i++) {
		this.htm += '<span id="'+id+'_no_'+i+'" style="padding:1px 5px 1px 5px;margin-left:1px;cursor:pointer;font-size:12px;color:#FFFFFF;'+(i == this.c ? 'background:#9E2688;' : 'background:#838B90;')+'">'+(i+1)+'</span>';
	}
	this.htm += '</div>';
	$('#'+id).html(this.htm);
	...
	this.slide = function(o) {
		...
		$('#'+id+'_no_'+o).css({background:'#9E2688'});
